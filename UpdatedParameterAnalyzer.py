...
def get_right_stride(prints_df, name):
    ...
    # 1. Filter for Hind Paws AND Right Paws only
    df = prints_df[(prints_df.is_hind) & (prints_df.is_right)]
    df = df.reset_index(drop=True)
    
    # 2. Check if we have enough prints (need at least 2 to measure 1 stride)
    if len(df) < 2:
        print(f'WARNING: Not enough Right prints for {name}')
        return np.nan

    # 3. Calculate distance between consecutive RIGHT prints (index i and i-1)
    xy = df.loc[:,['X','Y']].values
    # Note: range starts at 1 because we are looking at the previous same-side paw
    return np.mean([np.linalg.norm(xy[i,:] - xy[i-1,:]) for i in range(1, int(numbPrints/2))])

def get_left_stride(prints_df, name):
    numbPrints = hind_prints_to_analyze
    
    # 1. Filter for Hind Paws AND Left Paws only (~is_right)
    df = prints_df[(prints_df.is_hind) & (~prints_df.is_right)]
    df = df.reset_index(drop=True)
    
    # 2. Check for sufficient data
    if len(df) < 2:
        print(f'WARNING: Not enough Left prints for {name}')
        return np.nan
...
def make_day_file(subfolder):
    ...
    right_strides = [get_right_stride(dfs[i], file_names[i]) for i in range(len(dfs))]
    left_strides = [get_left_stride(dfs[i], file_names[i]) for i in range(len(dfs))]
    ...
    zz = pd.DataFrame( {'filename':file_names, 'Base of Support':avg_boses, 'Stride Length':avg_strides,
        'Right Stride Length':right_strides, 'Left Stride Length':left_strides, 
    ...
    output = zz[['filename', 
                 'Base of Support', 'Stride Length','Right Stride Length', 'Left Stride Length', 'Step Length',
